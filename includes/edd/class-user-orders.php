...
class User_Orders extends User_Order_Manager {
	...
	public function list_of_purchased_product_ids() {
		...
		foreach ( $this->get_customer_orders( $customer ) as $order ) {
	...
	private function get_edd_last_order( $customer ) {
		$orders = $this->get_customer_orders( $customer, 1 );

		return count( $orders ) ? $orders[0] : null;
	}

	/**
	 * Inspired by {@see EDD_Customer::get_payment_ids()}
	 *
	 * @param int $count Pass -1 to retrieve all orders
	 * @param EDD_Customer $customer
	 *
	 * @return Order[]
	 */
	private function get_customer_orders( $customer, $count = -1 ) {
		if ( -1 === $count ) {
			$count = edd_count_orders( array( 'customer_id' => $customer->id ) );
		}
		...
		return edd_get_orders(
			array(
				...
				// The following are the defaults, but `$this->get_edd_last_order()` relies on this order.
				// Therefore, explicitly list them here. Do not change them without considering the above method.
				'orderby'       => 'id',
				'order'         => 'DESC',
			)
		);
	...
	public static function remove_data() {
		...
		$wpdb->query( "DELETE FROM $wpdb->edd_ordermeta WHERE meta_key LIKE '_convesioconvert%'" );
	}
